        ...
        <Directory Id='Minsky' Name='Minsky'>
          <Directory Id='INSTALLDIR'>
            ...
            <Component Id='MinskyFiles' Guid='$componentId'>
              <File Id='MinskyEXE' Name='minsky.exe' Source='minsky.exe' KeyPath='yes'>
...
for i in *; do
	if [ ! -d $i -a $i != "minsky.exe" ]; then
	    let fid++
	    cat >>$minskyWxs <<EOF
	<File Id='fid$fid' Source='$i' KeyPath='no' />
EOF
        fi
done
	...
	if [ -f $i ]; then
	    let fid++
            if [ $j -eq 0 ]; then
 	        ...
 	        echo "<File Id='fid$fid' Source='$1/$i' KeyPath='yes' />">>$minskyWxs
           else
	        echo "<File Id='fid$fid' Source='$1/$i' KeyPath='no' />">>$minskyWxs
            fi
            let j++
	fi
    done
...
cat >>$minskyWxs <<EOF
    ...
    <Icon Id="minsky.exe" SourceFile="minsky.exe" />
